...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            Log.v("myApp", String.valueOf(jsonArray.length()));
            //Iterate the jsonArray and print the info of JSONObjects
            for(int i=0; i < jsonArray.length(); i++){

                JSONObject JSONObject = jsonArray.getJSONObject(i);
                JSONObject jsonSubObject = JSONObject.getJSONObject("properties");

                Log.v("myAppLoop",jsonSubObject.get("mag").toString());
                Log.v("myAppLoop",jsonSubObject.get("place").toString());
                Log.v("myAppLoop",df.format(Long.parseLong(jsonSubObject.get("time").toString())));
                Mag = jsonSubObject.get("mag").toString();
                Location = jsonSubObject.get("place").toString();
                DateOccured = df.format(Long.parseLong(jsonSubObject.get("time").toString()));


                earthquakes.add(new Earthquake(Mag,Location,DateOccured));
